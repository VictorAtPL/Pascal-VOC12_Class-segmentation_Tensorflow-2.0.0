...
class Model(AbstractModel):
    ...
    def get_model(self, **kwargs) -> keras.Model:
        ...
        pool2 = encoder.get_layer('block1_pool').output

        # Encoder
        conv6 = (Conv2D(filters=256, kernel_size=(7, 7), activation='relu', padding='same', name='block6_conv1'))(
            pool2)

        conv7 = (Conv2D(filters=256, kernel_size=(1, 1), activation='relu', padding='same', name='block7_conv1'))(
            conv6)

        # Decoder
        fcn32 = Conv2DTranspose(CLASS_NO, kernel_size=(7, 7), strides=(2, 2), padding='same', name='block10_deconv3',
